...
That is the approach I decided to follow, and the result was Ash. MTA exposes some very primative directX drawing functions, and Ash takes it from there.
...
Another thing Ash does better is that it decouples componenents from their renderers.
...
I use the word "Theme". The system is best explained through code. This is what a theme looks like in Ash.
	...
	do
		...
		function init.Test(self, themecontext)
			...
			//themecontext is an empty table that is availiable to the renderer as long as the component is to be rendered. It is discarded if the theme is changed or component is destroyed.
...
Another theme can further inherit from this Foobar theme, that's the usual way prototype inheritence works.
...
Ash uses a tree to propogate keyboard events, just like any other events. Moreover, the actual game itself (input to GTA San Andreas) is abstracted as an Ash component, which is a child of root.

Keyboard events are propogated only to the "active" child of the node ("active" is an iterator that is provided to Ash's event dispatcher, another use case of Ash's extra flexible event system).
...
This also means, that Ash's "GUI" is not contrained to 2D, or displaying at all. You could decide that a label should be represented as 3d textures being rendered in game. This means we can even reuse Ash's system for some sort of UI where the player is required to walk up to buttons to select them, etc. I will have to add a safety net here so that untrusted components can be run, but it looks promising.
...
Why stop at alpha? It also inherits all other colour transformations. You can tint elements, and all their children would get tinted. Colour transformations are accomplished by keeping 9x9 matrices, and multiplying them to perform affine transformations.
...
MTA also exposes methods to use SDL shaders with directX. I did not understand shaders when I made most of Ash, so I did not implement them as inherited context. Over the summer I have studied shaders and future versions of Ash will keep shaders as inherited context, making these colour transformations obsolete.
